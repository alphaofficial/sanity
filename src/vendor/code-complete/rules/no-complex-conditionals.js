    ...
    create(context) {
        ...
        const suggestName = (node) => {
            ...
            if (text.includes('enable') || text.includes('disable'))
                return 'isEnabled';
            if (text.includes('empty') || text.includes('length'))
                return 'hasItems';
            if (text.includes('error') || text.includes('fail'))
                return 'hasError';
            if (text.includes('ready') || text.includes('loaded'))
                return 'isReady';
            if (text.includes('allow') || text.includes('permit'))
                return 'isAllowed';
            return 'shouldProceed';
        };
        // Format condition breakdown for message
        const formatConditionBreakdown = (node) => {
            const sourceCode = getSourceCode(context);
            ...
            return 'Condition parts:\n' + parts.map((p, i) => p === 'AND' || p === 'OR' ? `  ${p}` : `  ${Math.floor(i / 2) + 1}. ${p}`).filter((_, i, arr) => arr[i] !== 'AND' && arr[i] !== 'OR' || true)
                .join('\n');
        };
        const checkCondition = (node, testNode) => {
            if (!testNode)
                return;
